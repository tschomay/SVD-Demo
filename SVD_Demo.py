...
# 3.
# Instead of noise, what if we modify the signal observed? Start by scaling
# the amplitude of each column of the data by a different amount.
...
# I'll make an example of sine waves with exponential decay over time.
signal_2 = np.exp([-x for x in range(20)])
data_dynamic = np.outer(signal, signal_2)
